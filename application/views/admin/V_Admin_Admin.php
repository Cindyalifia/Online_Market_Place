...
<?php $this->load->view('facade/modal', [
  'id' => 'modalAdd',
  'title' => 'Tambah',
  'form_action' => base_url('C_AdminAdmin/add'),
  'form_input' => [
    'Email' => form_input([
      'name' => 'email',
      'class' => 'form-control',
...
<?php 
  ...
  foreach ($data as $d) {
      ...
      'id' => 'modalUpdate'.$d['id'],
      'title' => 'Update',
      'form_action' => base_url('C_AdminAdmin/update'),
      'form_input' => [
      ...
      $('#tabelData tbody').on('click', '.delete-item', (e) => {
        swal({
          ...
          .then((willDelete) => {
            console.log(e);
            if (willDelete) {
              window.location.replace(`<?= base_url('C_AdminAdmin/delete')?>?id=${e.target.dataset.id}`);              
            }
          });
      })
